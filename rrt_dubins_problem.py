...
"""
    Support and testing for the RRT Dubins problem (problem 3).
"""

try:
    import dubins_path_planning
    from rrt_planning import planning
except ImportError:
    raise
...
class RRT_dubins_problem():
    ...
    """
    RRT_DUBINS_PROBLEM Class for RRT planning problems with dubins-type path
    ...
    def planning(self, display_map=False):
        """
            Calling function for your planning implementation
        """
        return planning(self, display_map=display_map)
...
def check_path(rrt_problem, path_node_list : list):
    ...
    # Condition 1: Check if the first and last nodes are the start and goal nodes
    if not (path_node_list[0].is_state_identical(rrt_problem.start) and
            path_node_list[-1].is_state_identical(rrt_problem.goal)):
        return False

    # Condition 2: Check if node[i-1] is the parent of node[i]
    for i in range(1, len(path_node_list)):
        if path_node_list[i].parent != path_node_list[i - 1]:
            return False

    # Condition 3: Check if the path is collision-free
    for node in path_node_list:
        if not rrt_problem.check_collision(node):
            return False

    # Condition 4: Check if node locations are inside the map area
    for node in path_node_list:
        x, y = node.x, node.y
        if (x < rrt_problem.x_lim[0] or x > rrt_problem.x_lim[1] or
                y < rrt_problem.y_lim[0] or y > rrt_problem.y_lim[1]):
            return False
...
def main():
    ...
    rrt_dubins = RRT_dubins_problem(start = start, goal = goal, \
                                    ...
                                    max_iter=200)
    path_node_list = rrt_dubins.planning(display_map=True)
